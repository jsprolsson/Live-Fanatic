  ...
  useEffect(() => {
    ...
    return (
      ...
      <div className="event-image">
        <img src={eventData.event_img_url} alt="artist image" />
      </div>
      ...
      <div className="right">
        <h2>{eventData.event_artist}</h2>
        <h3>{eventData.event_venue}</h3>
        <h4>
          {eventData.event_address}, {eventData.event_city}
        </h4>
        <p>
          <i>{eventData.event_description}</i>
        </p>
        <p>Age limit: {eventData.event_age_limit}</p>
        <p>Type of event: {eventData.event_type}</p>
        {eventData.event_type === "livestream" ? <div className="livestream-btn">
